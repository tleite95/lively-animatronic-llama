...
def analyze_record(record, cfg):
    ...
    unmapped = sorted(k for k in record if k not in used and k not in excluded and k not in ignored)
    return {
...
def analyze_key_catalog(keys, cfg):
    ...
    return {
        ...
        "unmapped_keys_present": sorted(
            k for k in keys if k not in used and k not in cfg["excluded_keys"] and not ignored_key(k, cfg)
        ),
    }
